...
$destination = '';
$invalid     = ''; // 초기에 변수가 정의되지 않아 발생하는 오류 방지하기 위해 추가
...
if ($id) {
  // 오타 수정: a.content. -> a.content, / JOIN ... - ... -> = / WHERE ... - :id: -> = :id
...
// 오타 수정: id. forename -> id, forename / member: -> member
$sql         = "SELECT id, forename, surname FROM member;";
$authors     = pdo($pdo, $sql) -> fetchAll();

// 오타 수정: id. name -> id, name / category: -> category
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $article['published'] =
        (isset($_POST['published']) and ($_POST['published'] == 1)) ? 1 : 0;
    ...
    // 키 이름 통일: 'member' -> 'author' (HTML 뷰단과 맞춤)
...
} else if ($_SERVER['REQUEST_METHOD'] == 'POST') { // POST 요청일 때만 실행하도록 보완
